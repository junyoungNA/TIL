...
//  리로딩 없이 필요한 부분만 서버로 부터 받아서 화면을 갱신하는 것이다.
...
// 👎 SPA 단점
// JavaScript 파일을 번들링해서 한 번에 받기 때문에 초기 구동 속도가
//  느리다. (Webpack의 code splitting으로 해결 가능)
...
// SSR이 렌더링하는 방식
// 웹에 시작부터 MPA가 있었다. 사실 MPA라는 용어도 SPA와 비교를 위해 등장한 인상이 강하다.
...
// 이 과정을 서버 사이드 렌더링(SSR)이라고 부른다.

// 서버 사이드 렌더링의 장점은 SEO이다. 전통적인 MPA의 경우 브라우저에서 JavaScript 코드가 동작하기 전에도 완성된 형태의 
// 템플릿 (HTML에 데이터가 삽입된 상태)을 서버로 부터 전달받는다. 이 때문에 검색로봇이 페이지를 크롤링하기에 매우 적합하다.

// CSR이 렌더링하는 방식
// 최초에 한번 서버에서 전체 페이지를 로딩하여 보여주고 이후에는 사용자의 요청이 올 때마다, 
// 리소스를 서버에서 제공한 후 클라이언트가 해석하고 렌더링하는 방식이다. SEO가 어렵다는 큰 단점이 있다.

// SPA를 SSR 방식으로 렌더링 할 수도 있다?! Next.js 를 활용해서
